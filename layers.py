...
def noam_norm(x, epsilon=1.0, scope=None):
...
def residual_block(inputs, num_blocks, num_conv_layers, kernel_size, mask=None,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_blocks):
            ...
            outputs, sublayer = conv_block(
                outputs, num_conv_layers, kernel_size, num_filters,
                scope="encoder_block_%d" % i, reuse=reuse,
                dropout=dropout, sublayers=(sublayer, total_sublayers))
            outputs, sublayer = self_attention_block(
                outputs, num_filters, seq_len, mask=mask, num_heads=num_heads,
...
def conv_block(inputs, num_conv_layers, kernel_size, num_filters,
               scope="conv_block", reuse=None, dropout=0.0, sublayers=(1, 1)):
    with tf.variable_scope(scope, reuse=reuse):
...
def self_attention_block(inputs, num_filters, seq_len, mask=None, num_heads=8,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs = tf.nn.dropout(outputs, 1.0 - dropout)
        outputs = multihead_attention(outputs, num_filters, num_heads=num_heads, reuse=reuse,
...
def multihead_attention(queries, units, num_heads, memory=None,
                        scope="Multi_Head_Attention", reuse=None, mask=None, bias=True, dropout=0.0):
    with tf.variable_scope(scope, reuse=reuse):
        # Self attention
        if memory is None:
            memory = queries
